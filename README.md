...
To make this production-ready, we had to build "guardrails" to handle four specific problems:
...
Hierarchy Rules: We gave the AI a "Rulebook." It's now instructed that the Main Offer belongs in the headlines, Benefits go in the paragraphs, and Actions go only on the buttons etc. This keeps the page's logic intact.
...
Surgical Reinjection: Instead of trying to find the text again by name (which might change), we "tattoo" every element with a hidden ID (data-ai-id). When the AI sends back a change for node_42, our code knows exactly which element that is, even if the text is now completely different and.
